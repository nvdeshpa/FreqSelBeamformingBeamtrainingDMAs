...
f_c=15*10^9;
lambda_c=c/f_c;
F= (lambda_c/3)^3; %% Coupling factor
Gamma=(2*pi*f_c)/50; %% Damping factor
B=6*10^9;
f_max= f_c+B/2;
f_min= f_c-B/2;
...
for phi_i=1:length(phi_t_arr_actual)
    ...
    for fi=1:length(fk_arr)
        f=fk_arr(fi);
        he_f= generate_intrinsic_phase_vector(f, ng, dy, Ny, c, atten_coeff, include_attenuation);
...
for tr_i=1:length(Tr_arr)
    ...
    for phi_i=1:length(phi_t_arr_actual)
        ...
        for bi=1:length(B_arr)
            BW=B_arr(bi);
            [rate_dma_starr_ft_star_phi_t(phi_i, bi, tr_i), rate_ttd(phi_i, bi, tr_i), rate_ps(phi_i, bi, tr_i)]= compute_achievable_rate_LOS_channel(BW, Kd, Ny, Nz, phi_t_actual,  f_r_l_m__exact_angle_known, f_center_exact_angle_known, F, Gamma, ng, dy, c, Power, N_0,  r,atten_coeff, include_attenuation,0 );
            [rate_dma_beam_trained(phi_i, bi, tr_i),~,~]=compute_achievable_rate_LOS_channel(BW, Kd, Ny, Nz, phi_t_actual,  f_r_l_m_beam_trained, f_center_beam_trained, F, Gamma, ng, dy, c, Power, N_0,  r, atten_coeff, include_attenuation, 0);
            [rate_dma_starr_fc_phi_t(phi_i, bi, tr_i), ~,~]=compute_achievable_rate_LOS_channel(BW, Kd, Ny, Nz, phi_t_actual, f_r_l_m_fixed_freq_fc, f_center_fixed, F, Gamma, ng, dy, c, Power, N_0,  r,atten_coeff, include_attenuation, 0 );
            [rate_dma_pin_fc(phi_i, bi, tr_i), ~,~]=compute_achievable_rate_LOS_channel(BW, Kd, Ny, Nz, phi_t_actual, f_r_l_m_pin, f_center_fixed, F, Gamma, ng, dy, c, Power, N_0,  r,atten_coeff, include_attenuation, 1, pin_config_arr_l_m );
        end
...
function [rate_DMA, rate_TTD, rate_PS]= compute_achievable_rate_LOS_channel(BW, Kd, Ny, Nz, phi,  f_r_l_m, f_center, F, Gamma, ng, dy, c, P, N_0,  r,atten_coeff, include_attenuation, varactor_or_pin, pin_config_arr_l_m  )
